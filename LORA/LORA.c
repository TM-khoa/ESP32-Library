...
lora_write_reg(int reg, int val)
{
   ...
   gpio_set_level(LoRaWAN_CS_GPIO, 0);
   spi_device_transmit(__spi, &t);
   gpio_set_level(LoRaWAN_CS_GPIO, 1);
}
...
lora_reset(void)
{
   gpio_set_level(LoRaWAN_RST_GPIO, 0);
   vTaskDelay(pdMS_TO_TICKS(1));
   gpio_set_level(LoRaWAN_RST_GPIO, 1);
...
lora_init(void)
{
   ...
   spi_bus_config_t bus = {
      .miso_io_num = LoRaWAN_MISO_GPIO,
      .mosi_io_num = LoRaWAN_MOSI_GPIO,
      .sclk_io_num = LoRaWAN_SCK_GPIO,
      .quadwp_io_num = -1,
      .quadhd_io_num = -1,
      .max_transfer_sz = 0
   };
   ...
   {
      ESP_LOGE(TAG, "Init spi device fail!");
   }
   /*
